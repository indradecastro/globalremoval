...
```{r}
raw <- raw[-grep(x=names(raw), pattern=c("dry|bag"))]
```
...
```{r dungaddition}
dungdif <- raw$final_dung_wet - raw$initial_dung
plot(dungdif, pch=c(1,19)[as.factor(dungdif>0)], col=c("black", "red")[as.factor(dungdif>0)]); abline(h=0)
raw[which(dungdif>0),1:6]
```
Dung increased during experiment, there was **NO** removal, but addition of dung!  

Moreover, the distribution of removal is not typical nor easy to analyze.  
I attempt to describe it as:  
*left-tailed or left-skewed normal distribution with zero inflation*
```{r removalhist}
head(merge(raw, H2O.loss[,c("codigo_parcela", "H2O.loss")]), 30)
# hist(dungdif/300*100, 100, xlim=c(-100,20), main="Percentage (%) of dung removed", xlab="% removed")
