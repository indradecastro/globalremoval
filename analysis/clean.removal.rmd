...
## Removal according to treatment
```{r}
...
usexp.pval <- ddply(usexp, .(ID), summarize,
                    "pval"=wilcox.test(rem48 ~ manejo)$p.value)
                    

usexp <- merge(usexp, usexp.pval)
usexp <- arrange(usexp, pval)
usexp$ID <- factor(usexp$ID, levels=unique(usexp$ID))
...
white.violins <- usexp %>% 
      ggplot() +
      aes(x=manejo, y=rem48) +
      geom_violin() +
      # geom_boxplot() + 
      facet_wrap(~ID, ncol=4) +
      labs(x="manejo", y="Removal rate 48h (gr/h)", title="Dung Removal in each treatment") +
      scale_y_continuous(expand = c(0,0), limits=c(0,4)) +
      geom_signif(comparisons = list(c("Int", "Ext")), map_signif_level=TRUE, test="wilcox.test")
